...
class ScreenshotRequest(BaseModel):
    ...
    url: HttpUrl = Field(..., description="URL to capture screenshot of")
    format: Literal["png", "jpeg", "webp"] = Field(
        default="png", description="Image format (png, jpeg, webp)"
    )
    width: int = Field(
        default=1280, description="Screenshot width in pixels", ge=1, le=5000
    )
    height: int = Field(
        default=720, description="Screenshot height in pixels", ge=1, le=5000
    )
...
class ScreenshotResponse(BaseModel):
    ...
    url: str = Field(..., description="URL to the processed image")
