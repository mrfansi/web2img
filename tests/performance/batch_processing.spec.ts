...
test.group('Batch Processing - Performance Tests', (group) => {
  ...
  group.each.teardown(async () => {
    ...
    try {
      ...
      console.warn('Failed to clean up batch jobs in batch processing performance test:', error)
    }
  })
  ...
  group.teardown(async () => {
    ...
    try {
      ...
      console.warn('Failed to clean up batch jobs in batch processing performance teardown:', error)
    }
  ...
  test('large batch processing performance - 100 items', async ({ assert }) => {
    ...
    do {
      ...
      if (Date.now() - lastLogTime > 30000) {
        console.log(
          `Progress: ${finalStatus.completed}/${itemCount} completed (${finalStatus.progress_percentage?.toFixed(1)}%)`
        )
  ...
  test('concurrent batch jobs performance', async ({ assert }) => {
    ...
    const totalCompleted = Object.values(jobStatuses).reduce(
      (sum: number, status: any) => sum + status.completed,
      0
    )
    const totalFailed = Object.values(jobStatuses).reduce(
      (sum: number, status: any) => sum + status.failed,
      0
    )
  ...
  test('batch processing with different parallelism levels', async ({ assert }) => {
    ...
    for (const parallelism of parallelismLevels) {
      ...
      console.log(
        `- Parallelism ${parallelism}: ${processingTime}ms, ${successRate.toFixed(1)}% success, ${itemsPerSecond.toFixed(2)} items/sec`
      )
    }
    ...
    results.forEach((result) => {
      console.log(
        `- Level ${result.parallelism}: ${result.processingTime}ms (${result.itemsPerSecond.toFixed(2)} items/sec, ${result.successRate.toFixed(1)}% success)`
      )
    })
    ...
    results.forEach((result) => {
      assert.isTrue(
        ...
        `Parallelism ${result.parallelism} should have at least 70% success rate`
      )
    })
...
// Helper functions
function analyzeProgressionRate(
  statusHistory: Array<{ timestamp: number; completed: number }>
): number {
...
function analyzeProcessingRate(
  statusHistory: Array<{ timestamp: number; completed: number; failed: number }>
): number {
  if (statusHistory.length < 2) return 0
