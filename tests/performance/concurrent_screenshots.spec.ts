...
test.group('Concurrent Screenshots - Performance Tests', (group) => {
  ...
  test('concurrent single screenshot requests - 10 requests', async ({ assert }) => {
    ...
    assert.isTrue(
      successfulResponses.length >= concurrency * 0.8,
  ...
  test('concurrent single screenshot requests - 25 requests', async ({ assert }) => {
    ...
    assert.isTrue(
      successfulResponses.length >= concurrency * 0.6,
  ...
  test('concurrent requests with cache performance', async ({ assert }) => {
    ...
    console.log(
      `- Cache hit rate: ${((cachedResponses.length / successfulResponses.length) * 100).toFixed(1)}%`
    )

    // Cache performance assertions
    assert.isTrue(
      successfulResponses.length >= concurrency * 0.9,
      'At least 90% should succeed with cache'
    )
    assert.isTrue(
      cachedResponses.length >= successfulResponses.length * 0.8,
      'At least 80% should be cache hits'
    )
