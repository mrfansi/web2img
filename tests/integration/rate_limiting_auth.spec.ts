...
test.group('Rate Limiting and Authentication - Integration Tests', (group) => {
  ...
  test('batch request rate limiting', async ({}) => {
    ...
    for (let i = 0; i < 3; i++) {
      const response = await apiClient
        ...
        .header('X-API-Key', lowLimitApiKey.key)
        .json({
          items: [{ id: `batch${i}-item1`, url: 'https://example.com' }],
        })
    ...
    for (let i = 0; i < 3; i++) {
      await apiClient
        ...
        .header('X-API-Key', lowLimitApiKey.key)
        .json({
          items: [{ id: `excess${i}-item1`, url: 'https://example.com' }],
        })
    }
    ...
    const rateLimitedResponse = await apiClient
      ...
      .header('X-API-Key', lowLimitApiKey.key)
      .json({
        items: [{ id: 'rate-limited-item', url: 'https://example.com' }],
      })
  ...
  test('authentication with missing API key', async ({}) => {
    ...
    const singleResponse = await apiClient.post('/screenshot').json({
    ...
    const batchResponse = await apiClient.post('/batch/screenshots').json({
      items: [{ id: 'no-auth-item', url: 'https://example.com' }],
    })
    ...
    const statusResponse = await apiClient.get('/batch/screenshots/123')
  ...
  test('authentication with invalid API key', async ({}) => {
    ...
    for (const invalidKey of invalidKeys) {
      const response = await apiClient.post('/screenshot').header('X-API-Key', invalidKey).json({
        url: 'https://example.com',
      })
