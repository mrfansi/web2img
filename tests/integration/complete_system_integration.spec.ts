...
test.group('Complete System Integration', (group) => {
  ...
  test('should get comprehensive system metrics', async ({ assert }) => {
    ...
    assert.isNumber(
      metrics.queues.screenshot.waiting,
      'Screenshot waiting count should be a number'
    )
  ...
  test('should complete full screenshot workflow', async ({ assert }) => {
    ...
    assert.include(
      ...
      'Should include ImgProxy URL generation step'
    )
  ...
  test('should process batch job with multiple items', async ({ assert }) => {
    ...
    const batchJob = await BatchJob.createBatchJob(testUrls.length, {
      parallel: 2,
      timeout: 30000,
      cache: true,
      fail_fast: false,
  ...
  test('should handle webhook delivery for batch completion', async ({ assert }) => {
    ...
    assert.isTrue(
      webhookService.validateWebhookUrl('https://example.com/webhook'),
      'Valid HTTPS URL should pass'
    )
    assert.isTrue(
      webhookService.validateWebhookUrl('http://example.com/webhook'),
      'Valid HTTP URL should pass'
    )
    assert.isFalse(
      webhookService.validateWebhookUrl('https://localhost/webhook'),
      'Localhost URL should fail'
    )
    assert.isFalse(
      webhookService.validateWebhookUrl('ftp://example.com/webhook'),
  ...
  test('should handle ImgProxy URL generation', async ({ assert }) => {
    ...
    if (isConfigured) {
      ...
      assert.include(
        generatedUrl,
        testImagePath,
        'Should include original path when not configured'
      )
    }
  })
